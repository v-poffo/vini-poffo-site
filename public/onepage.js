...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function renderAbout() {
        ...
        cardsToRender.forEach((c, i) => {
            ...
            const frontStyle = !isDesktop ? `data-img="${c.img}"` : '';
        ...
        if (!isDesktop) {
            const fronts = grid.querySelectorAll('.flip-card-front[data-img]');
                ...
                const io = new IntersectionObserver((entries) => {
                    entries.forEach(entry => {
                        if (entry.isIntersecting) {
                            ...
                            if (img) { el.style.backgroundImage = `url('${img}')`; el.style.backgroundAttachment = 'scroll'; el.removeAttribute('data-img'); }
                            io.unobserve(el);
                        }
                    });
                }, { rootMargin: '100px' });
                fronts.forEach(f => io.observe(f));
            } else {
                fronts.forEach(f => { f.style.backgroundImage = `url('${f.getAttribute('data-img')}')`; f.style.backgroundAttachment = 'scroll'; });
            }
        }
    }
